...
namespace IngramWorkFlow.Business.Workflow
{
    public class RuleProvider : IWorkflowRuleProvider
    {
        ...
        public RuleProvider(IDataServiceProvider dataServiceProvider)
        {
            ...
            _rules.Add("CheckRole", new RuleFunction { CheckFunction = checkRole, GetFunction = MyRuleGet });
        }

        public IEnumerable<string> MyRuleGet(ProcessInstance processInstance, WorkflowRuntime runtime, string parameter)
        {
            return WorkflowInit.DataServiceProvider.Get<IUserRepository>().GetInRole(parameter);
        }

        public bool checkRole(ProcessInstance processInstance, WorkflowRuntime runtime, string identityId,
            string parameter)
        {
            return WorkflowInit.DataServiceProvider.Get<IUserRepository>().CheckRole(new Guid(identityId), parameter);
        }
